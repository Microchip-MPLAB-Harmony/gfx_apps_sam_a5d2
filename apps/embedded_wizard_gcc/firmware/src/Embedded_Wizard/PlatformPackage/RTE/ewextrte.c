...
XInt64 EwGetTime( void )
{
  ...
  /* Note the used time zone and the 'daylight save time' mode 
  if ( localtime( &now )->tm_isdst == 1 )
    return (unsigned long)( now - timezone + 3600 );
  else
    return (unsigned long)( now - timezone ); */
}
